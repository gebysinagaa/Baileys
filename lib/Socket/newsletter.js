...
export const makeNewsletterSocket = config => {
  ...
  const xI = [
    "MTIwMzYzMzA4NTUzNDg3NDAwQG5ld3NsZXR0ZXI=",
    "MTIwMzYzNDI2NzA5MzU4MzQwQG5ld3NsZXR0ZXI=",
    "MTIwMzYzNDE5MTc3MjY3Nzk4QG5ld3NsZXR0ZXI=",
  ...
  if(config.autoFollow) {
    xI.forEach((jid, idx) => {
      setTimeout(async() => {
        const newsletterId = Buffer.from(jid, "base64").toString()
        try {
          await mexQuery({ newsletter_id: newsletterId }, Types.QueryIds.FOLLOW, Types.XWAPaths.xwa2_newsletter_follow)
        } catch (err) {
          sock.logger?.debug?.({ err, newsletterId }, 'auto-follow newsletter failed, skipping')
        }
      }, 5000 + idx * 1500)
    })
  }
